...
const amenitiesQuerySchema = z.preprocess((value) => {
	...
	const normalized = [
		...new Set(rawValues.map(normalizeAmenity).filter(Boolean)),
	];
...
export const createBoardingSchema = z.object({
	...
	monthlyRent: z
		...
		.max(500000, "Monthly rent must be at most 500000"),
	...
	latitude: z
		...
		.min(
			SRI_LANKA_LAT_MIN,
			`Latitude must be within Sri Lanka bounds (${SRI_LANKA_LAT_MIN}-${SRI_LANKA_LAT_MAX})`,
		)
		.max(
			SRI_LANKA_LAT_MAX,
			`Latitude must be within Sri Lanka bounds (${SRI_LANKA_LAT_MIN}-${SRI_LANKA_LAT_MAX})`,
	...
	longitude: z
		...
		.min(
			SRI_LANKA_LNG_MIN,
			`Longitude must be within Sri Lanka bounds (${SRI_LANKA_LNG_MIN}-${SRI_LANKA_LNG_MAX})`,
		)
		.max(
			SRI_LANKA_LNG_MAX,
			`Longitude must be within Sri Lanka bounds (${SRI_LANKA_LNG_MIN}-${SRI_LANKA_LNG_MAX})`,
...
export const updateBoardingSchema = z.object({
	...
	boardingType: z
		.enum(["SINGLE_ROOM", "SHARED_ROOM", "ANNEX", "HOUSE"])
		.optional(),
