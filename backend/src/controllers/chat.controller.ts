...
export async function createChatRoom(
  ...
  try {
    ...
    if (req.user.role !== otherUser.role) {
      ...
      ownerId = req.user.role === "OWNER" ? req.user.userId : body.otherUserId;
    } else {
      // If same role (e.g. STUDENT <-> STUDENT for marketplace), assign deterministically
      if (req.user.userId < body.otherUserId) {
        studentId = req.user.userId;
        ownerId = body.otherUserId;
      } else {
        studentId = body.otherUserId;
