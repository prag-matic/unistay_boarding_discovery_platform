...
export async function getIssues(
  ...
  try {
    ...
    const rawResult = hasMore ? issues.slice(0, Number(limit)) : issues;

    const result = rawResult.map((issue) => {
      const roomIdRaw = issue.roomId as unknown as Record<string, unknown>;
      return {
        ...issue,
        id: issue._id.toString(),
        roomId: roomIdRaw?._id
          ? {
              ...roomIdRaw,
              id: (roomIdRaw._id as { toString(): string }).toString(),
            }
          : issue.roomId,
      };
    });
    ...
    const nextCursor =
      hasMore && result.length > 0
        ? (
            result[result.length - 1] as unknown as { _id: Types.ObjectId }
          )._id.toString()
        : undefined;
