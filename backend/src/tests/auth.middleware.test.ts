...
describe('authenticate', () => {
  ...
  it('attaches user and calls next() for a valid token', () => {
    const token = signAccessToken({ userId: 'u1', role: 'STUDENT', email: 'a@b.com' });
  ...
  it('passes UnauthorizedError for a tampered token', () => {
    const token = signAccessToken({ userId: 'u1', role: 'OWNER', email: 'x@y.com' });
    const next = vi.fn();
    authenticate(req({ headers: { authorization: `Bearer ${token}tampered` } }), res, next);
    expect(next.mock.calls[0][0]).toBeInstanceOf(UnauthorizedError);
  });
});
