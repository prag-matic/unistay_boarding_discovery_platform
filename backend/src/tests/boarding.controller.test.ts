...
vi.mock('@/lib/prisma.js', () => {
  const db = {
    boarding: { findUnique: vi.fn(), findMany: vi.fn(), create: vi.fn(), update: vi.fn(), count: vi.fn() },
...
vi.mock('@/utils/slug.js', () => ({ generateUniqueSlug: vi.fn().mockResolvedValue('my-title') }));
vi.mock('@/lib/cloudinary.js', () => ({
  uploadBoardingImage: vi.fn().mockResolvedValue({ url: 'https://cdn/img.jpg', publicId: 'pid' }),
...
import {
  searchBoardings, getBoardingBySlug, getMyListings, createBoarding,
  updateBoarding, submitBoarding, deactivateBoarding, activateBoarding,
  uploadImages, deleteImage,
} from '@/controllers/boarding.controller.js';
import {
  BoardingNotFoundError, ValidationError, ForbiddenError, InvalidStateTransitionError,
} from '@/errors/AppError.js';

const db = prisma as any;

function mockReq(overrides: Record<string, unknown> = {}) {
  return { body: {}, params: {}, query: {}, user: { userId: 'owner1' }, files: undefined, ...overrides } as any;
}
function mockRes() {
  const res: any = {};
...
const fakeBoarding = {
  id: 'b1', ownerId: 'owner1', title: 'Nice Room', slug: 'nice-room',
  status: 'DRAFT', isDeleted: false, currentOccupants: 0, maxOccupants: 2,
  images: [], amenities: [], rules: [], owner: {},
};
...
describe('searchBoardings', () => {
  it('returns paginated active boardings', async () => {
    ...
    const res = mockRes();
    await searchBoardings(mockReq({ query: { page: 1, size: 10, sortBy: 'createdAt', sortDir: 'desc' } }), res, next);
  ...
  it('calls next on error', async () => {
    ...
    await searchBoardings(mockReq({ query: { page: 1, size: 10, sortBy: 'createdAt', sortDir: 'desc' } }), mockRes(), next);
...
describe('getBoardingBySlug', () => {
  it('returns boarding for active slug', async () => {
    db.boarding.findUnique.mockResolvedValue({ ...fakeBoarding, status: 'ACTIVE' });
    const res = mockRes();
    await getBoardingBySlug(mockReq({ params: { slug: 'nice-room' } }), res, next);
    expect(res.json).toHaveBeenCalled();
  });

  it('throws BoardingNotFoundError when not found', async () => {
    db.boarding.findUnique.mockResolvedValue(null);
    await getBoardingBySlug(mockReq({ params: { slug: 'x' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(BoardingNotFoundError));
  });

  it('throws BoardingNotFoundError when deleted', async () => {
    db.boarding.findUnique.mockResolvedValue({ ...fakeBoarding, isDeleted: true, status: 'ACTIVE' });
    await getBoardingBySlug(mockReq({ params: { slug: 'x' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(BoardingNotFoundError));
  });

  it('throws BoardingNotFoundError when not ACTIVE', async () => {
    db.boarding.findUnique.mockResolvedValue({ ...fakeBoarding, status: 'DRAFT' });
    await getBoardingBySlug(mockReq({ params: { slug: 'x' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(BoardingNotFoundError));
  });
});
...
describe('createBoarding', () => {
  it('creates boarding and returns 201', async () => {
    ...
    const res = mockRes();
    await createBoarding(mockReq({
      body: { title: 'Nice Room', city: 'Colombo', district: 'Colombo', address: 'x', monthlyRent: 10000, boardingType: 'HOSTEL', genderPref: 'ANY', maxOccupants: 2, currentOccupants: 0 },
    }), res, next);
...
describe('updateBoarding', () => {
  it('updates boarding and returns result', async () => {
    ...
    const res = mockRes();
    await updateBoarding(mockReq({ params: { id: 'b1' }, body: { title: 'Updated' } }), res, next);
    expect(res.json).toHaveBeenCalled();
  });

  it('throws BoardingNotFoundError when not found', async () => {
    db.boarding.findUnique.mockResolvedValue(null);
    await updateBoarding(mockReq({ params: { id: 'x' }, body: {} }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(BoardingNotFoundError));
  });

  it('throws ForbiddenError when not owner', async () => {
    db.boarding.findUnique.mockResolvedValue({ ...fakeBoarding, ownerId: 'other' });
    await updateBoarding(mockReq({ params: { id: 'b1' }, body: {} }), mockRes(), next);
...
describe('submitBoarding', () => {
  it('submits a DRAFT boarding with images', async () => {
    db.boarding.findUnique.mockResolvedValue({ ...fakeBoarding, status: 'DRAFT', images: [{ id: 'img1' }] });
    db.boarding.update.mockResolvedValue({ ...fakeBoarding, status: 'PENDING_APPROVAL' });
    const res = mockRes();
...
describe('deactivateBoarding', () => {
  it('deactivates an ACTIVE boarding', async () => {
    db.boarding.findUnique.mockResolvedValue({ ...fakeBoarding, status: 'ACTIVE' });
    db.boarding.update.mockResolvedValue({ ...fakeBoarding, status: 'INACTIVE' });
    const res = mockRes();
  ...
  it('throws ForbiddenError when not owner', async () => {
    db.boarding.findUnique.mockResolvedValue({ ...fakeBoarding, status: 'ACTIVE', ownerId: 'other' });
    await deactivateBoarding(mockReq({ params: { id: 'b1' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(ForbiddenError));
  });
...
describe('activateBoarding', () => {
  it('activates an INACTIVE boarding', async () => {
    db.boarding.findUnique.mockResolvedValue({ ...fakeBoarding, status: 'INACTIVE' });
...
describe('uploadImages', () => {
  it('uploads images and returns 201', async () => {
    ...
    db.$transaction.mockResolvedValue([{ id: 'img1', url: 'https://cdn/img.jpg', publicId: 'pid', createdAt: new Date() }]);
    const res = mockRes();
    const file = { buffer: Buffer.from('x'), mimetype: 'image/jpeg' } as any;
    await uploadImages(mockReq({ params: { id: 'b1' }, files: [file] }), res, next);
  ...
  it('throws BoardingNotFoundError when not found', async () => {
    db.boarding.findUnique.mockResolvedValue(null);
    await uploadImages(mockReq({ params: { id: 'x' }, files: [{}] }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(BoardingNotFoundError));
  });

  it('throws ForbiddenError when not owner', async () => {
    db.boarding.findUnique.mockResolvedValue({ ...fakeBoarding, ownerId: 'other', images: [] });
    await uploadImages(mockReq({ params: { id: 'b1' }, files: [{}] }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(ForbiddenError));
  });
  ...
  it('throws ValidationError when exceeding max images', async () => {
    const existingImages = Array.from({ length: 7 }, (_, i) => ({ id: `img${i}` }));
    db.boarding.findUnique.mockResolvedValue({ ...fakeBoarding, images: existingImages });
    const files = Array.from({ length: 3 }, () => ({ buffer: Buffer.from('x'), mimetype: 'image/jpeg' }));
    await uploadImages(mockReq({ params: { id: 'b1' }, files }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(ValidationError));
  });
});
...
describe('deleteImage', () => {
  it('deletes image', async () => {
    db.boarding.findUnique.mockResolvedValue(fakeBoarding);
    db.boardingImage.findUnique.mockResolvedValue({ id: 'img1', boardingId: 'b1', publicId: 'pid' });
    db.boardingImage.delete.mockResolvedValue({});
    const res = mockRes();
    await deleteImage(mockReq({ params: { id: 'b1', imageId: 'img1' } }), res, next);
  ...
  it('throws BoardingNotFoundError when boarding not found', async () => {
    db.boarding.findUnique.mockResolvedValue(null);
    await deleteImage(mockReq({ params: { id: 'x', imageId: 'img1' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(BoardingNotFoundError));
  });

  it('throws ForbiddenError when not owner', async () => {
    db.boarding.findUnique.mockResolvedValue({ ...fakeBoarding, ownerId: 'other' });
    await deleteImage(mockReq({ params: { id: 'b1', imageId: 'img1' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(ForbiddenError));
  });
  ...
  it('throws BoardingNotFoundError when image not found', async () => {
    ...
    await deleteImage(mockReq({ params: { id: 'b1', imageId: 'img99' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(BoardingNotFoundError));
  });
});
