...
import { sha256, generateSecureToken } from '@/lib/hash.js';
...
describe('sha256', () => {
  ...
  it('different inputs give different hashes', () => expect(sha256('a')).not.toBe(sha256('b')));
...
describe('generateSecureToken', () => {
  ...
  it('custom byte count', () => expect(generateSecureToken(48)).toHaveLength(96));
  it('generates unique tokens', () => expect(generateSecureToken()).not.toBe(generateSecureToken()));
});
