...
describe('validate (body)', () => {
  ...
  it('sends 422 and does not call next() on failure', () => {
    const res = mockRes();
    const next = vi.fn();
    validate(schema, 'body')(makeReq({ body: { name: '', age: -1 } }), res, next);
...
describe('validate (query)', () => {
  ...
  it('sends 422 for invalid query', () => {
    ...
    const res = mockRes();
    validate(qSchema, 'query')(makeReq({ query: { status: 'bad' } as any }), res, vi.fn());
    expect(res.status).toHaveBeenCalledWith(422);
  });
});
...
describe('shorthand helpers', () => {
  it('validateBody delegates correctly', () => {
    const next = vi.fn();
    validateBody(schema)(makeReq({ body: { name: 'Bob', age: 30 } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith();
  });
  ...
  it('validateQuery delegates correctly', () => {
    ...
    const next = vi.fn();
    validateQuery(qSchema)(makeReq({ query: { q: 'hi' } as any }), mockRes(), next);
    expect(next).toHaveBeenCalledWith();
  });
  ...
  it('validateParams delegates correctly', () => {
    ...
    const next = vi.fn();
    validateParams(pSchema)(makeReq({ params: { id: '1' } as any }), mockRes(), next);
    expect(next).toHaveBeenCalledWith();
  });
});
