...
describe("getMyVisitRequests", () => {
	it("returns student visit requests", async () => {
		...
		expect(db.visitRequest.findMany.mock.calls[0][0].where.studentId).toBe(
			"student1",
		);
	});
});
...
describe("getMyBoardingVisitRequests", () => {
	it("returns visit requests for owner boardings", async () => {
		db.visitRequest.findMany.mockResolvedValue([fakeVR]);
		const res = mockRes();
		await getMyBoardingVisitRequests(
		...
		expect(res.json).toHaveBeenCalled();
		expect(
			db.visitRequest.findMany.mock.calls[0][0].where.boarding.ownerId,
		).toBe("owner1");
	});
});
...
describe("getVisitRequestById", () => {
	it("returns visit request for the student", async () => {
		db.visitRequest.findUnique.mockResolvedValue({
			...fakeVR,
			studentId: "student1",
		});
		const res = mockRes();
