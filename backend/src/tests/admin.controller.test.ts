...
import {
  listUsers, getUserById, activateUser, deactivateUser,
  listPendingBoardings, approveBoarding, rejectBoarding,
...
function mockReq(overrides: Record<string, unknown> = {}) {
  return { body: {}, params: {}, query: {}, user: { userId: 'admin1', role: 'ADMIN' }, ...overrides } as any;
}
function mockRes() {
  const res: any = {};
...
const fakeUser = { id: 'u1', email: 'a@b.com', firstName: 'A', lastName: 'B', role: 'STUDENT', isActive: true };
const fakeBoarding = { id: 'b1', title: 'Room', status: 'PENDING_APPROVAL', isDeleted: false };
...
describe('listPendingBoardings', () => {
  it('returns pending boardings', async () => {
    ...
    expect(res.json).toHaveBeenCalled();
    expect(db.boarding.findMany.mock.calls[0][0].where.status).toBe('PENDING_APPROVAL');
  });
});
...
describe('rejectBoarding', () => {
  it('rejects a PENDING_APPROVAL boarding', async () => {
    db.boarding.findUnique.mockResolvedValue(fakeBoarding);
    db.boarding.update.mockResolvedValue({ ...fakeBoarding, status: 'REJECTED' });
    const res = mockRes();
    await rejectBoarding(mockReq({ params: { id: 'b1' }, body: { reason: 'Bad listing' } }), res, next);
    expect(res.json).toHaveBeenCalled();
  ...
  it('throws BoardingNotFoundError when not found', async () => {
    db.boarding.findUnique.mockResolvedValue(null);
    await rejectBoarding(mockReq({ params: { id: 'x' }, body: { reason: 'r' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(BoardingNotFoundError));
  });

  it('throws InvalidStateTransitionError when not PENDING_APPROVAL', async () => {
    db.boarding.findUnique.mockResolvedValue({ ...fakeBoarding, status: 'ACTIVE' });
    await rejectBoarding(mockReq({ params: { id: 'b1' }, body: { reason: 'r' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(InvalidStateTransitionError));
  });
});
