...
describe("saveBoarding", () => {
	it("saves boarding and returns 201", async () => {
		...
		db.savedBoarding.create.mockResolvedValue({
			id: "s1",
			boardingId: "b1",
			studentId: "student1",
			createdAt: new Date(),
...
describe("getSavedBoardings", () => {
	it("returns saved boardings for student", async () => {
		db.savedBoarding.findMany.mockResolvedValue([
			{ id: "s1", createdAt: new Date(), boarding: activeBoarding },
		]);
		const res = mockRes();
		await getSavedBoardings(mockReq(), res, next);
		expect(res.json).toHaveBeenCalled();
		expect(db.savedBoarding.findMany.mock.calls[0][0].where.studentId).toBe(
			"student1",
		);
	});
});
