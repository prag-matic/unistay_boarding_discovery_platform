...
describe("InvalidCredentialsError", () => {
	it("has 404", () =>
		expect(new InvalidCredentialsError().statusCode).toBe(404));
});
...
describe("InvalidStateTransitionError", () => {
	it("has 422", () =>
		expect(new InvalidStateTransitionError().statusCode).toBe(422));
});
...
describe("NotFoundError", () => {
	it("default resource name", () =>
		expect(new NotFoundError().message).toBe("Resource not found"));
	it("custom resource name", () =>
		expect(new NotFoundError("Boarding").message).toBe("Boarding not found"));
});

describe("BadRequestError", () => {
	it("has 400", () => expect(new BadRequestError().statusCode).toBe(400));
	it("accepts custom message", () =>
		expect(new BadRequestError("oops").message).toBe("oops"));
});
