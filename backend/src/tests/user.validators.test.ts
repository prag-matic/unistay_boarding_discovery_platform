...
import { updateProfileSchema, changePasswordSchema, adminListUsersQuerySchema } from '@/schemas/user.validators.js';
...
describe('updateProfileSchema', () => {
  ...
  it('rejects lastName over 100 chars', () =>
    expect(() => updateProfileSchema.parse({ lastName: 'a'.repeat(101) })).toThrow());
});

describe('changePasswordSchema', () => {
  const valid = { currentPassword: 'OldPass1', newPassword: 'NewPass1' };

  it('accepts valid data', () => expect(() => changePasswordSchema.parse(valid)).not.toThrow());

  it('rejects empty currentPassword', () =>
    expect(() => changePasswordSchema.parse({ ...valid, currentPassword: '' })).toThrow());

  it('rejects newPassword < 8 chars', () =>
    expect(() => changePasswordSchema.parse({ ...valid, newPassword: 'Short1' })).toThrow());

  it('rejects newPassword without uppercase', () =>
    expect(() => changePasswordSchema.parse({ ...valid, newPassword: 'password1' })).toThrow());

  it('rejects newPassword without number', () =>
    expect(() => changePasswordSchema.parse({ ...valid, newPassword: 'Password' })).toThrow());
});
...
describe('adminListUsersQuerySchema', () => {
  ...
  it('rejects invalid role', () =>
    expect(() => adminListUsersQuerySchema.parse({ role: 'SUPERUSER' })).toThrow());
