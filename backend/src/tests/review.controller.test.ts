...
describe("createReview", () => {
	...
	it("throws BadRequestError when userId missing", async () => {
		await createReview(
			mockReq({
				body: { data: JSON.stringify({ boardingId: BOARDING_ID, rating: 4 }) },
				user: undefined,
				headers: {},
			}),
			mockRes(),
...
describe("getReviewsByBoarding", () => {
	it("returns paginated reviews", async () => {
		...
		const res = mockRes();
		await getReviewsByBoarding(
			mockReq({
				params: { boardingId: "b1" },
		...
		expect(res.json).toHaveBeenCalled();
		expect(svc.getReviewsByBoarding).toHaveBeenCalledWith(
			"b1",
...
describe("updateReview", () => {
	it("updates review", async () => {
		...
		await updateReview(
			mockReq({
				params: { id: "r1" },
				body: { data: JSON.stringify({ rating: 5, comment: "Updated" }) },
			}),
			res,
			next,
		);
	...
	it('maps "Review not found" to NotFoundError', async () => {
		...
		await updateReview(
			mockReq({
				params: { id: "x" },
				body: { data: JSON.stringify({ rating: 5 }) },
			}),
			mockRes(),
			next,
		);
	...
	it("maps ownership error to ForbiddenError", async () => {
		svc.updateReview.mockRejectedValue(
			new Error("You can only edit your own reviews"),
		);
		await updateReview(
			mockReq({
				params: { id: "r1" },
				body: { data: JSON.stringify({ rating: 3 }) },
			}),
			mockRes(),
			next,
		);
...
describe("deleteReview", () => {
	...
	it("maps ownership error to ForbiddenError", async () => {
		svc.deleteReview.mockRejectedValue(
	...
	it("throws BadRequestError when userId missing", async () => {
		await deleteReview(
			mockReq({ params: { id: "r1" }, user: undefined, headers: {} }),
			mockRes(),
			next,
		);
		expect(next).toHaveBeenCalledWith(expect.any(BadRequestError));
...
describe("addReviewReaction", () => {
	it("adds a reaction", async () => {
		...
		await addReviewReaction(
			mockReq({ params: { id: "r1" }, body: { type: "LIKE" } }),
			res,
			next,
		);
		expect(res.json).toHaveBeenCalled();
...
describe("createReviewComment", () => {
	it("creates a comment and returns 201", async () => {
		...
		await createReviewComment(
			mockReq({ params: { id: "r1" }, body: { comment: "Nice" } }),
			res,
			next,
		);
		expect(res.status).toHaveBeenCalledWith(201);
...
describe("updateReviewComment", () => {
	it("updates a comment", async () => {
		...
		await updateReviewComment(
			mockReq({ params: { id: "c1" }, body: { comment: "Updated" } }),
			res,
			next,
		);
		expect(res.json).toHaveBeenCalled();
	...
	it('maps "Comment not found" to NotFoundError', async () => {
		...
		await updateReviewComment(
			mockReq({ params: { id: "x" }, body: { comment: "x" } }),
			mockRes(),
			next,
		);
		expect(next).toHaveBeenCalledWith(expect.any(NotFoundError));
	...
	it("maps ownership error to ForbiddenError", async () => {
		svc.updateReviewComment.mockRejectedValue(
			new Error("You can only edit your own comments"),
		);
		await updateReviewComment(
			mockReq({ params: { id: "c1" }, body: { comment: "x" } }),
			mockRes(),
			next,
		);
		expect(next).toHaveBeenCalledWith(expect.any(ForbiddenError));
...
describe("deleteReviewComment", () => {
	...
	it("throws BadRequestError when userId missing", async () => {
		await deleteReviewComment(
			mockReq({ params: { id: "c1" }, user: undefined, headers: {} }),
			mockRes(),
			next,
		);
		expect(next).toHaveBeenCalledWith(expect.any(BadRequestError));
...
describe("addReviewCommentReaction", () => {
	it("adds a reaction to a comment", async () => {
		svc.addReviewCommentReaction.mockResolvedValue({
			action: "added",
			type: "LIKE",
		});
		const res = mockRes();
		await addReviewCommentReaction(
			mockReq({ params: { id: "c1" }, body: { type: "LIKE" } }),
			res,
			next,
		);
		expect(res.json).toHaveBeenCalled();
