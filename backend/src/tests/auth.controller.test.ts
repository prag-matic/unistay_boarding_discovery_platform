...
vi.mock('@/lib/prisma.js', () => {
  const db = {
    ...
    passwordResetToken: { create: vi.fn(), findUnique: vi.fn(), delete: vi.fn(), deleteMany: vi.fn(), update: vi.fn() },
    $transaction: vi.fn(),
  };
...
vi.mock('@/lib/hash.js', () => ({ generateSecureToken: vi.fn().mockReturnValue('rawtoken'), sha256: vi.fn().mockReturnValue('hashedtoken') }));
vi.mock('@/lib/email.js', () => ({ sendVerificationEmail: vi.fn(), sendPasswordResetEmail: vi.fn() }));
vi.mock('@/lib/jwt.js', () => ({ signAccessToken: vi.fn().mockReturnValue('access-jwt'), parseDurationMs: vi.fn().mockReturnValue(86400000) }));
vi.mock('@/utils/index.js', () => ({ sanitizeUser: vi.fn((u: Record<string, unknown>) => { const { passwordHash: _, ...s } = u as any; return s; }) }));

import { prisma } from '@/lib/prisma.js';
import bcrypt from 'bcryptjs';
import { register, login, refreshToken, logout, verifyEmail, resendVerification, forgotPassword, resetPassword } from '@/controllers/auth.controller.js';
import { UserAlreadyExistsError, InvalidCredentialsError, AccountDeactivatedError, UnauthorizedError, UserNotFoundError, TokenExpiredError } from '@/errors/AppError.js';

const db = prisma as any;

function mockReq(overrides: Record<string, unknown> = {}) {
  return { body: {}, params: {}, query: {}, user: undefined, ...overrides } as any;
}
function mockRes() {
  const res: any = {};
...
const fakeUser = { id: 'u1', email: 'a@b.com', firstName: 'A', lastName: 'B', passwordHash: 'hashed', role: 'STUDENT', isActive: true, isVerified: false };
...
describe('register', () => {
  it('creates user and sends 201', async () => {
    ...
    const res = mockRes();
    await register(mockReq({ body: { email: 'a@b.com', password: 'Pass1234', firstName: 'A', lastName: 'B', role: 'STUDENT' } }), res, next);
  ...
  it('throws UserAlreadyExistsError when email taken', async () => {
    ...
    await register(mockReq({ body: { email: 'a@b.com', password: 'x' } }), mockRes(), next);
...
describe('login', () => {
  it('returns tokens for valid credentials', async () => {
    ...
    const res = mockRes();
    await login(mockReq({ body: { email: 'a@b.com', password: 'Pass1234' } }), res, next);
  ...
  it('throws InvalidCredentialsError when user not found', async () => {
    db.user.findUnique.mockResolvedValue(null);
    await login(mockReq({ body: { email: 'x@b.com', password: 'p' } }), mockRes(), next);
  ...
  it('throws InvalidCredentialsError on bad password', async () => {
    ...
    await login(mockReq({ body: { email: 'a@b.com', password: 'wrong' } }), mockRes(), next);
    expect(next).toHaveBeenCalled();
  });
  ...
  it('throws AccountDeactivatedError when inactive', async () => {
    ...
    (bcrypt.compare as any).mockResolvedValue(true);
    await login(mockReq({ body: { email: 'a@b.com', password: 'Pass1234' } }), mockRes(), next);
    expect(next).toHaveBeenCalled();
  });
});
...
describe('refreshToken', () => {
  it('issues new tokens', async () => {
    db.refreshToken.findUnique.mockResolvedValue({ id: 'rt1', userId: 'u1', revokedAt: null });
    db.user.findUnique.mockResolvedValue({ ...fakeUser, isActive: true });
    ...
    const res = mockRes();
    await refreshToken(mockReq({ body: { refreshToken: 'rawtoken' } }), res, next);
    expect(res.json).toHaveBeenCalled();
  ...
  it('throws UnauthorizedError for revoked token', async () => {
    ...
    await refreshToken(mockReq({ body: { refreshToken: 'bad' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(UnauthorizedError));
  });

  it('throws UnauthorizedError when token not found', async () => {
    db.refreshToken.findUnique.mockResolvedValue(null);
    await refreshToken(mockReq({ body: { refreshToken: 'gone' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(UnauthorizedError));
  });
  ...
  it('throws UserNotFoundError when user missing', async () => {
    db.refreshToken.findUnique.mockResolvedValue({ id: 'rt1', userId: 'u1', revokedAt: null });
    db.user.findUnique.mockResolvedValue(null);
    await refreshToken(mockReq({ body: { refreshToken: 'x' } }), mockRes(), next);
...
describe('verifyEmail', () => {
  it('verifies email and returns success', async () => {
    ...
    db.emailVerificationToken.findUnique.mockResolvedValue({ id: 'ev1', userId: 'u1', expiresAt: future, token: 'tok' });
  ...
  it('throws TokenExpiredError and deletes when expired', async () => {
    ...
    db.emailVerificationToken.findUnique.mockResolvedValue({ id: 'ev1', expiresAt: past, token: 'tok' });
...
describe('resendVerification', () => {
  it('sends email for valid unverified user', async () => {
    ...
    db.emailVerificationToken.create.mockResolvedValue({});
    const res = mockRes();
    await resendVerification(mockReq({ body: { email: 'a@b.com' } }), res, next);
    expect(res.json).toHaveBeenCalled();
  });
  ...
  it('returns generic message when user not found', async () => {
    db.user.findUnique.mockResolvedValue(null);
    const res = mockRes();
    await resendVerification(mockReq({ body: { email: 'x@b.com' } }), res, next);
    expect(res.json).toHaveBeenCalled();
...
describe('resetPassword', () => {
  it('resets password successfully', async () => {
    ...
    db.passwordResetToken.findUnique.mockResolvedValue({ id: 'pr1', userId: 'u1', used: false, expiresAt: future });
    db.$transaction.mockResolvedValue([{}, {}, {}]);
    const res = mockRes();
    await resetPassword(mockReq({ body: { token: 'tok', password: 'NewPass1' } }), res, next);
    expect(res.json).toHaveBeenCalled();
  });

  it('throws TokenExpiredError for invalid/used token', async () => {
    db.passwordResetToken.findUnique.mockResolvedValue({ id: 'pr1', used: true, expiresAt: new Date(Date.now() + 1000) });
    await resetPassword(mockReq({ body: { token: 'tok', password: 'x' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(TokenExpiredError));
  });

  it('throws TokenExpiredError and deletes expired token', async () => {
    const past = new Date(Date.now() - 1000);
    db.passwordResetToken.findUnique.mockResolvedValue({ id: 'pr1', used: false, expiresAt: past });
  ...
  it('throws TokenExpiredError when token not found', async () => {
    ...
    await resetPassword(mockReq({ body: { token: 'x', password: 'y' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(TokenExpiredError));
  });
});
