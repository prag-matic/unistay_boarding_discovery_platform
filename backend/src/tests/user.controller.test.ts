...
vi.mock('bcryptjs', () => ({ default: { hash: vi.fn().mockResolvedValue('newHash'), compare: vi.fn() } }));
vi.mock('@/lib/cloudinary.js', () => ({ uploadProfileImage: vi.fn() }));

import { prisma } from '@/lib/prisma.js';
import bcrypt from 'bcryptjs';
import { uploadProfileImage } from '@/lib/cloudinary.js';
import { getMe, updateMe, changePassword, uploadProfileImageHandler } from '@/controllers/user.controller.js';
import { UnauthorizedError, UserNotFoundError, InvalidCredentialsError } from '@/errors/AppError.js';

const db = prisma as any;

function mockReq(overrides: Record<string, unknown> = {}) {
  return { body: {}, params: {}, user: { userId: 'u1' }, file: undefined, ...overrides } as any;
}
function mockRes() {
  const res: any = {};
...
const fakeUser = { id: 'u1', email: 'a@b.com', firstName: 'A', lastName: 'B', passwordHash: 'hash', role: 'STUDENT', isActive: true };
...
describe('changePassword', () => {
  it('changes password on valid current password', async () => {
    ...
    const res = mockRes();
    await changePassword(mockReq({ body: { currentPassword: 'old', newPassword: 'NewPass1' } }), res, next);
    expect(db.user.update).toHaveBeenCalledOnce();
    expect(res.json).toHaveBeenCalled();
  });
  ...
  it('throws UserNotFoundError when user not in DB', async () => {
    db.user.findUnique.mockResolvedValue(null);
    await changePassword(mockReq({ body: { currentPassword: 'x', newPassword: 'y' } }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(UserNotFoundError));
  });
  ...
  it('throws InvalidCredentialsError for wrong current password', async () => {
    ...
    await changePassword(mockReq({ body: { currentPassword: 'wrong', newPassword: 'x' } }), mockRes(), next);
...
describe('uploadProfileImageHandler', () => {
  it('uploads image and updates user', async () => {
    ...
    db.user.update.mockResolvedValue({ ...fakeUser, profileImageUrl: 'https://cdn/img.jpg' });
  ...
  it('throws UnauthorizedError when no req.user', async () => {
    await uploadProfileImageHandler(mockReq({ user: undefined }), mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(UnauthorizedError));
  });

  it('throws ValidationError when no file provided', async () => {
    const { ValidationError } = await import('@/errors/AppError.js');
    await uploadProfileImageHandler(mockReq({ file: undefined }), mockRes(), next);
