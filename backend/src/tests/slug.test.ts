...
describe('buildSlug', () => {
  it('converts title to kebab-case with 6-char suffix', () => {
    expect(buildSlug('My Boarding House')).toMatch(/^my-boarding-house-[a-z0-9]{6}$/);
  });
...
describe('generateUniqueSlug', () => {
  ...
  it('skips collision when the existing record has the excluded ID', async () => {
    vi.doMock('@/lib/prisma.js', () => ({
      default: {
        boarding: { findUnique: vi.fn().mockResolvedValue({ id: 'my-id', slug: 's' }) },
      },
    }));
    const { generateUniqueSlug } = await import('@/utils/slug.js');
  ...
  it('throws AppError after max attempts', async () => {
    vi.doMock('@/lib/prisma.js', () => ({
      default: {
        boarding: { findUnique: vi.fn().mockResolvedValue({ id: 'other', slug: 'taken' }) },
      },
    }));
    const { generateUniqueSlug } = await import('@/utils/slug.js');
    await expect(generateUniqueSlug('Always Taken')).rejects.toThrow('Could not generate a unique slug');
  });
});
