...
export default function ManageReservationsScreen() {
  ...
  const loadReservations = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to load reservation applications. Please try again.');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleApprove = (id: string, boardingTitle: string) => {
    Alert.alert(
      'Approve Reservation',
      `Approve this reservation for "${boardingTitle}"? This will increment occupancy and generate 12 rental periods.`,
      [
          ...
          <View style={styles.studentRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              {item.student.university && (
                <Text style={styles.studentUniversity}>{item.student.university}</Text>
              )}
            </View>
          </View>
        )}
      ...
      {item.status === 'PENDING' && (
        <View style={styles.actionRow}>
          ...
          <TouchableOpacity
            ...
            onPress={() => handleApprove(item.id, item.boarding.title)}
            disabled={isActing}
          >
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Reservation Applications</Text>
        <View style={{ width: 40 }} />
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyTitle}>No reservation applications</Text>
              <Text style={styles.emptySub}>
                Students will send reservation applications to your properties here
              </Text>
            </View>
          }
        />
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <Text style={styles.modalSubtitle}>
              Please provide a reason for rejecting this reservation application.
            </Text>
            <TextInput
...
const styles = StyleSheet.create({
  ...
  studentUniversity: { fontSize: 11, color: COLORS.primary, marginTop: 1 },
