...
type TimestampedConfig = InternalAxiosRequestConfig & {
...
export async function uploadMyProfileImage(uri: string): Promise<string> {
  ...
  const response = await api.put<
    UniStayApiResponse<{ profileImageUrl: string }>
...
api.interceptors.request.use(
  (config) => {
    ...
    if (config.params)
      payload.params = sanitize(config.params) as Record<string, unknown>;
    if (
      config.data &&
  ...
  (error) => {
    logger.api.error("Request setup error", {
...
api.interceptors.response.use(
  (response) => {
    ...
    const duration = config._requestStartTime
      ? Date.now() - config._requestStartTime
      : undefined;
    const method = (config.method ?? "GET").toUpperCase();
    const url = config.url ?? "";
    const { data } = response;
...
api.interceptors.response.use(
  (response) => response,
  async (error) => {
    const originalRequest = error.config as TimestampedConfig & {
      _retry?: boolean;
    };
    ...
    if (status === 401 && !originalRequest._retry && !isRefreshEndpoint) {
      logger.api.info(
        `Token expired for ${method} ${url} – attempting token refresh`,
      );
